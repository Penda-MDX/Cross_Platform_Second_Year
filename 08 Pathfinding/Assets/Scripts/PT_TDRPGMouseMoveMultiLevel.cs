...
public class PT_TDRPGMouseMoveMultiLevel : MonoBehaviour {
    ...
    void Start()
    {
        ...
        nextMoves = new Vector2[NumberOFLevels];
        CurrentStatus = "Idle";
    }
    ...
    Vector2 ReachedCurrent()
    {
        //copy the array skipping the first - reached item
        Vector2[] _tmpMoveNodes = new Vector2[NumberOFLevels];

        for(int nodeCounter = 0; nodeCounter< NumberOFLevels; nodeCounter++)
        {
            if (nextMoves[nodeCounter + 1] != Vector2.zero)
            {
                _tmpMoveNodes[nodeCounter] = nextMoves[nodeCounter + 1];
            }
            else
            {
                if (nodeCounter == 0)
                {
                    _tmpMoveNodes[nodeCounter] = FindNextTarget(transform.position);
                }
                else
                {
                    _tmpMoveNodes[nodeCounter] = FindNextTarget(_tmpMoveNodes[nodeCounter--]);
                }
                
            }            
        }
        nextMoves = _tmpMoveNodes;
        return nextMoves[0];
    }

    Vector2 FindNextTarget(Vector2 startingPoint)
    {
